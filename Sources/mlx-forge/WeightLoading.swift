...
/// Resolve a DFlash2 drafter (`incoai/Qwen3.8-27B-DFlash2` and kin) for a
/// Qwen3.5-family target. Explicit configuration wins; otherwise accept a
/// sidecar inside the target folder, a checkpoint under the target's own
/// models root, or one in Forge's managed model cache. A drafter matches when
/// its `dflash_config` object exists and its hidden size, target depth and
/// vocabulary equal the target's.
func dflash2DrafterDirectory(for targetDirectory: URL) -> URL? {
    let fm = FileManager.default
    guard let targetConfig = qwenConfiguration(at: targetDirectory),
        qwenModelType(in: targetConfig).hasPrefix("qwen3_5"),
        qwenModelType(in: targetConfig) != "qwen3_5_mtp"
    ...
    func isCompatibleDrafter(_ url: URL) -> Bool {
        guard let draftConfig = qwenConfiguration(at: url),
            draftConfig["dflash_config"] is [String: Any]
        else { return false }
        return draftConfig["hidden_size"] as? Int == targetText["hidden_size"] as? Int
            && draftConfig["num_target_layers"] as? Int == targetText["num_hidden_layers"] as? Int
            && draftConfig["vocab_size"] as? Int == targetText["vocab_size"] as? Int
    }

    if let explicit = ProcessInfo.processInfo.environment["FORGE_DFLASH2_DRAFTER_PATH"],
        !explicit.isEmpty
    {
        let url = URL(fileURLWithPath: explicit, isDirectory: true)
        if isCompatibleDrafter(url) { return url }
    ...
    for name in ["dflash2-drafter", "DFlash2"] {
        let url = targetDirectory.appending(component: name, directoryHint: .isDirectory)
        if isCompatibleDrafter(url) { return url }
    }

    // The target's models root is two levels up (`<root>/<org>/<name>`);
    // check its `<org>/<name>` folders without walking into checkpoints.
    let targetRoot = targetDirectory.deletingLastPathComponent().deletingLastPathComponent()
    let orgs =
        (try? fm.contentsOfDirectory(
            at: targetRoot, includingPropertiesForKeys: [.isDirectoryKey],
            options: [.skipsHiddenFiles])) ?? []
    for org in orgs {
        let names =
            (try? fm.contentsOfDirectory(
                ...
                options: [.skipsHiddenFiles])) ?? []
        for name in names where isCompatibleDrafter(name) {
            return name
        }
    }
...
/// A loaded DFlash2 drafter. Drafters hold no per-stream state (the iterator
/// owns it), and Forge calls one only from inside its target container's
/// serialized `perform`, so sharing the reference across isolation domains
/// is sound.
final class DFlash2DrafterBox: @unchecked Sendable {
    let model: any DFlash2DrafterModel
    let directory: URL

    init(model: any DFlash2DrafterModel, directory: URL) {
        self.model = model
        self.directory = directory
    }
}

/// Load the DFlash2 drafter at `directory` through the drafter factory.
func loadDFlash2Drafter(
    directory: URL, tokenizerLoader: any TokenizerLoader
) async throws -> DFlash2DrafterBox {
    await DFlash2Registration.register()
    let context = try await DFlash2DrafterModelFactory.shared._load(
        configuration: ResolvedModelConfiguration(directory: directory),
        tokenizerLoader: tokenizerLoader)
    _ = stackSameInputProjections(in: context.model)
    return DFlash2DrafterBox(model: context.model, directory: directory)
}
