...
# 3. No unconditional plaintext HTTP endpoints except loopback. ForgeServer has
#    an explicit user-controlled LAN exposure mode, so its dynamic display and
#    Host/Origin allowlist URLs are permitted.
http_hits="$(grep -rn 'http://' "$SRC" --include='*.swift' 2>/dev/null \
  | grep -vE 'http://(127\.0\.0\.1|localhost|::1)([:/]|$)' \
  | grep -vE 'Sources/mlx-forge/ForgeServer\.swift:[0-9]+:.*(LocalNetwork\.lanHosts|hostNames\.map)' \
  | grep -vE ':[0-9]+:[[:space:]]*//')"
if [[ -n "$http_hits" ]]; then
  red "No unconditional non-loopback http:// URLs"; echo "$http_hits" | sed 's/^/     /'; fail=1
else
  grn "No unconditional non-loopback http:// URLs"
fi
