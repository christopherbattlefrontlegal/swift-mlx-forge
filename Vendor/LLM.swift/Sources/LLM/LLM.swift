...
open class LLM: ObservableObject {
    ...
    /// Boxes a Swift closure so llama.cpp's C progress callback can reach it.
    private final class LoadProgressBox {
        let callback: (Float) -> Bool
        init(_ callback: @escaping (Float) -> Bool) { self.callback = callback }
    }
    ...
    public init?(
        ...
        #endif
        let progressBox = loadProgress.map(LoadProgressBox.init)
        if let progressBox {
            modelParams.progress_callback = { fraction, userData in
                guard let userData else { return true }
                return Unmanaged<LoadProgressBox>.fromOpaque(userData)
                    .takeUnretainedValue().callback(fraction)
            }
            modelParams.progress_callback_user_data =
                Unmanaged.passUnretained(progressBox).toOpaque()
        }
        let cPath = self.path
        let loaded = withExtendedLifetime(progressBox) {
