...
public struct DFlash2SpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    private mutating func prepare(
        input: LMInput, prefilledPrefixTokens: Int, prefillStepSize: Int
    ) {
        processor?.prompt(input.text.tokens)
        let promptTokens = input.text.tokens
        let promptLength = promptTokens.dim(0)
        if prefilledPrefixTokens > 0, let attention = cache.first(where: { $0 is KVCacheSimple }) {
            precondition(
        ...
        let stepSize = Swift.max(1, prefillStepSize)
