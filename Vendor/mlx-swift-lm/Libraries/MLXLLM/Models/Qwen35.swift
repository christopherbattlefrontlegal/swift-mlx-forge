...
public struct Qwen35TextConfiguration: Codable, Sendable {
    ...
    // Native Qwen MTP fields. Qwen3.6 sidecars commonly expose
    // `mtp_num_hidden_layers`; older Qwen3-Next configs use
    // `num_nextn_predict_layers`.
    var numNextnPredictLayers: Int = 0
    var mtpNumHiddenLayers: Int? = nil
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.mtpNumHiddenLayers = try container.decodeIfPresent(
            Int.self, forKey: .mtpNumHiddenLayers)
        self.numNextnPredictLayers = try container.decodeIfPresent(
            Int.self, forKey: .numNextnPredictLayers) ?? self.mtpNumHiddenLayers ?? 0
...
final class Qwen35NativeMTPLayer: Module {
    ...
    init(_ args: Qwen35TextConfiguration) {
        ...
        _layers.wrappedValue = [
            Qwen35DecoderLayer(args, layerIdx: args.fullAttentionInterval - 1)
        ]
    ...
    func callAsFunction(
        ...
        var value = fc(concatenated([
            preFCNormEmbedding(embedding), preFCNormHidden(hidden),
        ], axis: -1))
        let mask = createAttentionMask(h: value, cache: cache)
        for layer in layers {
            value = layer(
                value,
                attentionMask: mask,
...
public class Qwen35TextModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    @ModuleInfo(key: "mtp") var nativeMTP: [Qwen35NativeMTPLayer]
    ...
    public init(_ args: Qwen35TextConfiguration) {
        ...
        let mtpCount = QwenNativeMTPConfig.retainWeights ? args.numNextnPredictLayers : 0
        _nativeMTP.wrappedValue = (0 ..< mtpCount).map { _ in
            Qwen35NativeMTPLayer(args)
        }
    }
